  ...
  <div>
    <v-card :width="app.config.width" class="text-center" :style="{ backgroundColor: app.config.color }">
      ...
      <v-list v-if="$root.data.apps.Todo.todo.length > 0">
        <v-list-item v-for="(item, index) in $root.data.apps.Todo.todo" :key="index">
          <v-checkbox @dblclick="deleteItem(index)" dense class="ma-0" :label="item.title" v-model="item.done"></v-checkbox>
        </v-list-item>
      </v-list>
      ...
      <v-card-actions class="py-0" :style="{ backgroundColor: app.config.color }">
        <v-text-field @keypress.enter="saveItem()" label="New Item" v-model="new_item"></v-text-field>
