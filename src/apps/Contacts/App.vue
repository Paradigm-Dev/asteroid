      ...
      <v-container class="pa-0" fluid>
        <v-row>
          <v-col sm="4" class="py-0">
            <v-list dense nav class="text-left" height="300" style="overflow: scroll;">
              <v-list-item-group mandatory v-model="current_contact">
                <v-list-item :value="contact" v-for="(contact, index) in $root.data.apps.Contacts.contacts" :key="index">
          ...
          <v-col sm="8" v-if="current_contact" class="pr-9">
            <v-text-field v-model="current_contact.name" label="Name" @change="$saveData()"></v-text-field>
            <v-text-field v-model="current_contact.phone" label="Phone Number" @change="$saveData()"></v-text-field>
            <v-text-field v-model="current_contact.email" label="Email Address" @change="$saveData()"></v-text-field>
            <v-btn icon color="red" @click="deleteContact()"><v-icon>mdi-delete</v-icon></v-btn>
            <p>{{ current_contact.index }}</p>
          </v-col>
        </v-row>
      </v-container>

      <v-card-actions class="py-0" :style="{ backgroundColor: app.config.color }">
        <v-text-field @keypress.enter="saveContact()" label="New Contact" v-model="new_contact"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    deleteContact() {
      ...
      var index = this.$root.data.apps.Contacts.contacts.findIndex(findContactIndex)
      console.log(index)
