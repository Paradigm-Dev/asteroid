<template>
  <div class="setup index" style="height: calc(100vh - 88px)">
    <v-card
      width="1000"
      class="window text-center"
      style="background: rgba(132, 0, 187, 0.8)"
    >
      <v-system-bar
        height="32"
        style="background-color: rgba(132, 0, 187, 0.8)"
      >
        <span class="__style-appname">Apollo Installer</span>
        <v-spacer></v-spacer>
        <v-btn x-small icon @click="refreshMoveable()"
          ><v-icon>mdi-reload</v-icon></v-btn
        >
        <v-btn x-small icon @click="cancel_popup = true"
      ...
      <v-card-actions style="background-color: rgba(132, 0, 187, 0.8)">
        <v-btn
          :disabled="page >= 4 || page <= 1"
          @click="page >= 8 || page <= 1 ? console.log() : (page -= 1)"
          text
    ...
    <v-card
      v-if="cancel_popup"
      width="350"
      class="window-confirm text-center"
      style="background: rgba(154, 0, 0, 0.8)"
    >
        ...
        <v-row>
          ...
          <v-col sm="8" class="text-left">
            ...
            <span class="font-weight-light grey--text text--lighten-2"
              >Are you sure you want to cancel the installation?</span
            >
          </v-col>
        </v-row>
      </v-container>
